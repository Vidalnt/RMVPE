    ...
    def __getitem__(self, index):
        ...
        if n < min_n:
            ...
            audio = torch.nn.functional.pad(
                data_buffer["audio"], (0, pad_s), mode="constant"
            )

            if data_buffer["noise"] is not None:
                noise = torch.nn.functional.pad(
                    data_buffer["noise"], (0, pad_s), mode="constant"
                )
            else:
                noise = None

            cent = torch.nn.functional.pad(
                data_buffer["cent"], (0, pad_n), mode="constant"
            )
            voice = torch.nn.functional.pad(
                data_buffer["voice"], (0, pad_n), mode="constant"
            )
            n = min_n
        else:
        ...
        mel = self.mel(
            audio_aug.unsqueeze(0), keyshift=key_shift, center=False
        ).squeeze(0)
        ...
        pitch_label = torch.exp(
            -(
                (
                    torch.arange(N_CLASS).expand(end_frame - start_frame, -1)
                    - index.unsqueeze(-1)
                )
                ** 2
            )
        ...
        with open(label_path, "r") as f:
            ...
            for line in lines:
                ...
                if float(line) != 0 and i < n_frames:
                    cent[i] = 1200 * np.log2(float(line) / 10)
                    voice[i] = 1
...
class MIR1K(Dataset):
    ...
    def __getitem__(self, index):
        ...
        start_frame = (
            0 if self.whole_audio else random.randint(1, data_buffer["len"] - 128)
        )
        ...
        cent = data_buffer["cent"][start_frame:end_frame] + 1200 * np.log2(
            win_length_new / WINDOW_LENGTH
        )
